...
function ProjCard(props) {
	return (
		<div
			...
			style={
				props.customColor /* custom color is optional */ && {
					"--main-card-color": props.customColor,
				}
			}
		>
			<Image
      ...
      Therefore, the conditional operator returns an empty string when props.buttonlink is undefined */}
			<Link href={props.buttonlink ? props.buttonlink : ""} passHref>
				<div className={classes["card-btn-container"]}>
					<button className={classes["card-btn"]}>{props.buttontext}</button>
				</div>
			</Link>
		</div>
	);
